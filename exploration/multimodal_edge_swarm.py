...
class FusionCenter:
    ...
    def generate_fusion_rule(self, historical_data: Dict) -> Dict:
        """生成融合规则"""
        # 基于历史性能调整权重
        performance_weights = {}
        for sensor_type, data in historical_data.items():
            if 'accuracy' in data:
                performance_weights[sensor_type] = data['accuracy']
            else:
                performance_weights[sensor_type] = 0.5
        
        # 归一化权重
        total = sum(performance_weights.values())
        if total > 0:
            for sensor_type in performance_weights:
                performance_weights[sensor_type] /= total
        
        return performance_weights


class MultimodalEdgeSwarm:
    """多模态边缘智能群"""
    ...
    def __init__(self, config: MultimodalConfig, seed: int = 42):
        ...
        # 创建融合中心
        self.fusion_center = FusionCenter(config, seed)
    ...
    def select_best_nodes(self, top_k: int = 2) -> List[MultimodalSensorNode]:
        """选择表现最好的节点"""
    ...
    def train(self, sensor_data: Dict, global_epochs: int = 30):
        """训练多模态边缘智能群"""
        print(f"\n开始训练多模态边缘智能群 ({len(self.nodes)} 节点)")
        print("="*70)
...
def test_multimodal_swarm():
    ...
    # 创建边缘群
    swarm = MultimodalEdgeSwarm(config)
...
def plot_results(performance: List[float], fusion_accuracy: List[float], test_results: Dict):
    ...
    plt.tight_layout()
    plt.savefig('f:/sel-lab/exploration/multimodal_results.png', dpi=150, bbox_inches='tight')
    print("\n✓ 结果图已保存: f:/sel-lab/exploration/multimodal_results.png")
    plt.show()


if __name__ == "__main__":
    test_multimodal_swarm()
