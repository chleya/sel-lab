# -*- coding: utf-8 -*-
"""
Evaluate optimizer-level two-speed updates against current-path boost.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_two_speed_boost_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_TWO_SPEED_BOOST_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    ...
    for task_suite in DIGITS_SUITES:
        print("\n" + "=" * 60)
        print(f"Phase 3 Two-Speed Boost Benchmark: suite={task_suite}")
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_two_speed_boost_{task_suite}_results.json",
...
def render_markdown(payload: Dict) -> str:
    lines = [
        ...
        f"- Shared tuned configuration: current-path boost `{payload['current_path_lr_boost']:.2f}`, old-path LR scale `{payload['old_path_lr_scale']:.2f}`",
...
def main() -> None:
    ...
    print(f"Saved two-speed boost benchmark JSON: {TARGET_JSON}")
    print(f"Saved two-speed boost benchmark markdown: {TARGET_MD}")
