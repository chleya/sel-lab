# -*- coding: utf-8 -*-
"""
Compare a minimal hybrid boost mechanism against plasticity- and interference-focused baselines.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_hybrid_boost_diagnostic.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_HYBRID_BOOST_DIAGNOSTIC.md"
...
def run_benchmark() -> Dict:
    ...
    for task_suite in TASK_SUITES:
        print("\n" + "=" * 60)
        print(f"Phase 3 Hybrid Boost Diagnostic: suite={task_suite}")
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_hybrid_boost_{task_suite}.json",
            policies=POLICIES,
        )
...
def main() -> None:
    ...
    print(f"Saved hybrid boost diagnostic JSON: {TARGET_JSON}")
    print(f"Saved hybrid boost diagnostic markdown: {TARGET_MD}")
