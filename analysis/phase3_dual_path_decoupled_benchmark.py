# -*- coding: utf-8 -*-
"""
Evaluate whether decoupling current-path task loss from memory-path inference
improves the dual-path boost recipe on the broader digits benchmark family.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_dual_path_decoupled_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_DUAL_PATH_DECOUPLED_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    ...
    for task_suite in DIGITS_SUITES:
        print("\n" + "=" * 60)
        print(f"Phase 3 Dual Path Decoupled Benchmark: suite={task_suite}")
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_dual_path_decoupled_{task_suite}_results.json",
...
def main() -> None:
    ...
    print(f"Saved dual-path decoupled benchmark JSON: {TARGET_JSON}")
    print(f"Saved dual-path decoupled benchmark markdown: {TARGET_MD}")
