# -*- coding: utf-8 -*-
"""
Preserve the sparse boost branch from the two-stage selector and learn quadratic heads only for default and embedded regions.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_guarded_router_selector_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_GUARDED_ROUTER_SELECTOR_BENCHMARK.md"
...
def build_config(
    ...
    cfg.selector_guarded_router_default_fit_weights = tuple(
        default_head.get("weights", (0.0,) * TASK_REGIME_QUADRATIC_FEATURE_COUNT)
    )
    cfg.selector_guarded_router_default_fit_bias = float(default_head.get("bias", 0.0))
    cfg.selector_guarded_router_embedded_fit_weights = tuple(
        embedded_head.get("weights", (0.0,) * TASK_REGIME_QUADRATIC_FEATURE_COUNT)
    )
    cfg.selector_guarded_router_embedded_fit_bias = float(embedded_head.get("bias", 0.0))
    return cfg


def guarded_pair_scale(benchmark_name: str, region_name: str) -> float:
    scale = 1.0
    if benchmark_name == "selector_adversarial_gate" and region_name == "embedded":
        scale *= 1.8
    if benchmark_name == "selector_sparse_adversarial_gate":
        scale *= 0.8
    return float(scale)
...
def fit_selector_payload() -> Dict[str, object]:
    ...
    region_feature_blocks = {"default": [], "embedded": []}
    region_target_blocks = {"default": [], "embedded": []}
    region_scale_blocks = {"default": [], "embedded": []}
    benchmark_rows = []
    for benchmark_name in benchmark_names:
        ...
        for idx, (feature_row, utility) in enumerate(zip(features, utilities)):
            ...
            if region_name == "sparse_embedded":
                continue
            pair_scale = guarded_pair_scale(benchmark_name, region_name)
            ...
            task_rows[-1]["pair_scale"] = float(pair_scale)
    ...
    region_payloads = {}
    for region_name in ("default", "embedded"):
...
def run_benchmark() -> Dict:
    ...
    for benchmark_name in benchmark_names:
        ...
        print("\n" + "=" * 60)
        print(f"Phase 3 Guarded Router Selector Benchmark: {benchmark_name} ({task_suite})")
        print("=" * 60)
        result = run_phase3_ablations(
            ...
            result_filename=f"phase3_guarded_router_selector_{benchmark_name}_{task_suite}.json",
            policies=policies,
        )
...
def render_markdown(payload: Dict) -> str:
    lines = [
        "# Phase 3 Guarded Router Selector Benchmark",
        "",
        "Guarded segmented router that preserves the hard sparse boost branch and learns quadratic heads only for default and embedded regions.",
        "",
...
def main() -> None:
    ...
    print(f"Saved guarded router selector benchmark JSON: {TARGET_JSON}")
    print(f"Saved guarded router selector benchmark markdown: {TARGET_MD}")
