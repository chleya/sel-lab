# -*- coding: utf-8 -*-
"""
Compare quantized internal specialists against quantized external memory on broader digits.
"""
...
QUANT_BITS = 4
TARGET_JSON = resolve_canonical_results_path("phase3_quantized_memory_path_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_QUANTIZED_MEMORY_PATH_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    ...
    for task_suite in DIGITS_SUITES:
        print("\n" + "=" * 60)
        print(f"Phase 3 Quantized Memory Path Benchmark: suite={task_suite}, bits={QUANT_BITS}")
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_quantized_memory_path_{task_suite}_results.json",
...
def render_markdown(payload: Dict) -> str:
    lines = [
        ...
        f"- Archived memory precision: `{payload['quant_bits']}`-bit",
    ...
    lines.extend(["", "## Suite Winners", ""])
    for row in payload["rows"]:
        lines.append(
            f"- `{row['task_suite']}`: best `{row['best_policy']}`, "
            f"internal={row['task_specialist_clone_current_path_boost_merge']['avg_accuracy_delta_vs_adapt_only']:+.1%}, "
            f"external={row['task_specialist_clone_external_memory_boost_merge']['avg_accuracy_delta_vs_adapt_only']:+.1%}"
        )
    lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    ...
    print(f"Saved quantized memory-path benchmark JSON: {TARGET_JSON}")
    print(f"Saved quantized memory-path benchmark markdown: {TARGET_MD}")
