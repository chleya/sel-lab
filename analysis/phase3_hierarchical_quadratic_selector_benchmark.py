# -*- coding: utf-8 -*-
"""
Fit region-specific quadratic ranking heads behind the existing selector segmentation and benchmark them on selector_full_map.
"""
...
from analysis.phase3_hierarchical_selector_benchmark import (
    REGION_NAMES,
    build_config as build_linear_config,
...
TARGET_JSON = resolve_canonical_results_path("phase3_hierarchical_quadratic_selector_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_HIERARCHICAL_QUADRATIC_SELECTOR_BENCHMARK.md"


def expand_quadratic_features(feature_row: np.ndarray) -> np.ndarray:
    pairwise_terms = []
    for idx in range(len(feature_row)):
        for jdx in range(idx, len(feature_row)):
            pairwise_terms.append(feature_row[idx] * feature_row[jdx])
    return np.concatenate([feature_row, np.array(pairwise_terms, dtype=float)])
...
def build_config(
    ...
    cfg = build_linear_config(task_suite, {}, baseline_payloads)
    ...
    cfg.selector_hierarchical_quadratic_default_fit_weights = tuple(
        default_head.get("weights", (0.0,) * TASK_REGIME_QUADRATIC_FEATURE_COUNT)
    )
    cfg.selector_hierarchical_quadratic_default_fit_bias = float(default_head.get("bias", 0.0))
    cfg.selector_hierarchical_quadratic_embedded_fit_weights = tuple(
        embedded_head.get("weights", (0.0,) * TASK_REGIME_QUADRATIC_FEATURE_COUNT)
    )
    cfg.selector_hierarchical_quadratic_embedded_fit_bias = float(embedded_head.get("bias", 0.0))
    cfg.selector_hierarchical_quadratic_sparse_fit_weights = tuple(
        sparse_head.get("weights", (0.0,) * TASK_REGIME_QUADRATIC_FEATURE_COUNT)
    )
    cfg.selector_hierarchical_quadratic_sparse_fit_bias = float(sparse_head.get("bias", 0.0))
    return cfg
...
def run_benchmark() -> Dict:
    ...
    for benchmark_name in benchmark_names:
        ...
        print("\n" + "=" * 60)
        print(f"Phase 3 Hierarchical Quadratic Selector Benchmark: {benchmark_name} ({task_suite})")
        print("=" * 60)
        result = run_phase3_ablations(
            ...
            result_filename=f"phase3_hierarchical_quadratic_selector_{benchmark_name}_{task_suite}.json",
            policies=policies,
        )
...
def render_markdown(payload: Dict) -> str:
    lines = [
        ...
        f"- Quadratic feature count: `{payload['selector_payload']['quadratic_feature_count']}`",
        "",
        "## Region Heads",
        "",
    ]
...
def main() -> None:
    ...
    print(f"Saved hierarchical quadratic selector benchmark JSON: {TARGET_JSON}")
    print(f"Saved hierarchical quadratic selector benchmark markdown: {TARGET_MD}")
