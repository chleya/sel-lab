# -*- coding: utf-8 -*-
"""
Benchmark a diagnostic-driven regime selector against the hardcoded regime switch and single-policy baselines.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_diagnostic_selector_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_DIAGNOSTIC_SELECTOR_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    payload = run_policy_benchmark_family(
        ...
        result_prefix="phase3_diagnostic_selector",
        title="Phase 3 Diagnostic Selector Benchmark",
        config_overrides={
...
def render_markdown(payload: Dict) -> str:
    lines = [
        "# Phase 3 Diagnostic Selector Benchmark",
        "",
        "Task-diagnostic-driven regime selector benchmarked against hardcoded regime switching and single-policy baselines.",
        "",
        f"- Family: `{payload['family_name']}`",
        f"- Overall best policy: `{payload['overall_best_policy']}`",
        f"- Selector thresholds: conflict `{payload['selector_config']['selector_conflict_threshold']:.2f}`, confidence `{payload['selector_config']['selector_confidence_threshold']:.2f}`",
        "",
        "## Aggregate",
        "",
    ]
...
def main() -> None:
    ...
    print(f"Saved diagnostic selector benchmark JSON: {TARGET_JSON}")
    print(f"Saved diagnostic selector benchmark markdown: {TARGET_MD}")
