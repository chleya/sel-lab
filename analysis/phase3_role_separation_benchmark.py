# -*- coding: utf-8 -*-
"""
Evaluate a role-separation specialist policy on the broader digits benchmark family.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_role_separation_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_ROLE_SEPARATION_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    ...
    for task_suite in DIGITS_SUITES:
        print("\n" + "=" * 60)
        print(f"Phase 3 Role Separation Benchmark: suite={task_suite}")
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_role_sep_{task_suite}_results.json",
...
def render_markdown(payload: Dict) -> str:
    lines = [
        "# Phase 3 Role Separation Benchmark",
        "",
    ...
    lines.extend(
        [
            ...
            "## Suite Winners",
            "",
            "| Task Suite | Best Policy | limited_merge | distill_merge | role_separation_merge |",
            "|---|---|---:|---:|---:|",
        ]
    )
    for row in payload["rows"]:
        lines.append(
            ...
            f"{row['task_specialist_clone_role_separation_merge']['avg_accuracy_delta_vs_adapt_only']:+.1%} |"
        )
    lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    ...
    print(f"Saved role separation benchmark JSON: {TARGET_JSON}")
    print(f"Saved role separation benchmark markdown: {TARGET_MD}")
