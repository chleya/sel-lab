# -*- coding: utf-8 -*-
"""
Evaluate asymmetric specialist retention updates on the broader digits benchmark family.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_asymmetric_retention_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_ASYMMETRIC_RETENTION_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    ...
    for task_suite in DIGITS_SUITES:
        print("\n" + "=" * 60)
        print(f"Phase 3 Asymmetric Retention Benchmark: suite={task_suite}")
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_asymmetric_retention_{task_suite}_results.json",
...
def render_markdown(payload: Dict) -> str:
    lines = [
        ...
        f"- Shared tuned configuration: merge scale `{payload['merge_scale']:.2f}`, distill strength `{payload['distill_strength']:.2f}`, retention strength `{payload['retention_strength']:.2f}`",
    ...
    for row in payload["rows"]:
        lines.append(
            f"| {row['task_suite']} | {row['best_policy']} | "
            f"{row['task_specialist_clone_limited_merge']['avg_accuracy_delta_vs_adapt_only']:+.1%} | "
            f"{row['task_specialist_clone_distill_merge']['avg_accuracy_delta_vs_adapt_only']:+.1%} | "
            f"{row['task_specialist_clone_asymmetric_retention_merge']['avg_accuracy_delta_vs_adapt_only']:+.1%} |"
        )
    lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    ...
    print(f"Saved asymmetric retention benchmark JSON: {TARGET_JSON}")
    print(f"Saved asymmetric retention benchmark markdown: {TARGET_MD}")
