# -*- coding: utf-8 -*-
"""
Compare weight-space archived-memory prototypes against existing low-budget baselines.
"""
...
REPRESENTATION_CASES = (
    ("full", 0, "recency"),
    ("recency_2", 2, "recency"),
    ("weight_prototype_2", 2, "weight_prototype"),
)
QUANT_BITS = 4
TARGET_JSON = resolve_canonical_results_path("phase3_internal_memory_weight_prototype_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_INTERNAL_MEMORY_WEIGHT_PROTOTYPE_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    ...
    for task_suite in DIGITS_SUITES:
        ...
        print("\n" + "=" * 60)
        print(
            "Phase 3 Internal Memory Weight-Prototype Benchmark: "
            f"suite={task_suite}, bits={QUANT_BITS}, budget=2"
        )
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_internal_memory_weight_prototype_2_{task_suite}_results.json",
            policies=("adapt_only", "task_specialist_clone_current_path_boost_merge"),
        )
        suite_row["weight_prototype_2"] = result["policies"]["task_specialist_clone_current_path_boost_merge"]["summary"]
...
def main() -> None:
    ...
    print(f"Saved internal-memory weight-prototype benchmark JSON: {TARGET_JSON}")
    print(f"Saved internal-memory weight-prototype markdown: {TARGET_MD}")
