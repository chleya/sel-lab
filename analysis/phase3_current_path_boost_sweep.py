# -*- coding: utf-8 -*-
"""
Sweep current-path learning-rate boost on the broader digits benchmark family.
"""
...
BOOST_VALUES = (1.25, 1.50, 1.75, 2.00)
TARGET_JSON = resolve_exploratory_results_path("phase3_current_path_boost_sweep.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_CURRENT_PATH_BOOST_SWEEP.md"
...
def run_sweep() -> Dict:
    ...
    for boost in BOOST_VALUES:
        ...
        for task_suite in DIGITS_SUITES:
            print("\n" + "=" * 60)
            print(f"Phase 3 Current Path Boost Sweep: boost={boost:.2f}, suite={task_suite}")
            print("=" * 60)
            result = run_phase3_ablations(
                config=build_config(task_suite, boost),
                result_filename=f"phase3_current_path_boost_{str(boost).replace('.', '_')}_{task_suite}.json",
                policies=(
                    "adapt_only",
...
def render_markdown(payload: Dict) -> str:
    best = payload["best"]
    lines = [
        "# Phase 3 Current Path Boost Sweep",
        "",
        f"- Digits suites: `{', '.join(payload['task_suites'])}`",
        f"- Best current-path LR boost: `{best['current_path_lr_boost']:.2f}`",
...
def main() -> None:
    ...
    print(f"Saved current-path boost sweep JSON: {TARGET_JSON}")
    print(f"Saved current-path boost sweep markdown: {TARGET_MD}")
