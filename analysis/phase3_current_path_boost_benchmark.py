# -*- coding: utf-8 -*-
"""
Evaluate current-path learning-rate boost on the broader digits benchmark family.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_current_path_boost_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_CURRENT_PATH_BOOST_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    ...
    for task_suite in DIGITS_SUITES:
        print("\n" + "=" * 60)
        print(f"Phase 3 Current Path Boost Benchmark: suite={task_suite}")
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_current_path_boost_{task_suite}_results.json",
...
def render_markdown(payload: Dict) -> str:
    lines = [
        ...
        f"- Shared tuned configuration: merge scale `{payload['merge_scale']:.2f}`, distill strength `{payload['distill_strength']:.2f}`, current-path LR boost `{payload['current_path_lr_boost']:.2f}`",
    ...
    for row in payload["rows"]:
        lines.append(
            ...
            f"{row['task_specialist_clone_current_path_boost_merge']['avg_accuracy_delta_vs_adapt_only']:+.1%} |"
        )
    lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    ...
    print(f"Saved current-path boost benchmark JSON: {TARGET_JSON}")
    print(f"Saved current-path boost benchmark markdown: {TARGET_MD}")
