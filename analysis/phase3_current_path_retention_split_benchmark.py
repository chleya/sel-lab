# -*- coding: utf-8 -*-
"""
Evaluate a dual-path split variant that boosts the current path while freezing archived specialists into retention-only updates.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_current_path_retention_split_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_CURRENT_PATH_RETENTION_SPLIT_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    ...
    for task_suite in DIGITS_SUITES:
        print("\n" + "=" * 60)
        print(f"Phase 3 Current Path Retention Split Benchmark: suite={task_suite}")
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_current_path_retention_split_{task_suite}_results.json",
...
def main() -> None:
    ...
    print(f"Saved current-path retention split benchmark JSON: {TARGET_JSON}")
    print(f"Saved current-path retention split benchmark markdown: {TARGET_MD}")
