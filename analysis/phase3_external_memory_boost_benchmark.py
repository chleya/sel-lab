# -*- coding: utf-8 -*-
"""
Evaluate a truly separate external-memory reuse path against current-path boost.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_external_memory_boost_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_EXTERNAL_MEMORY_BOOST_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    ...
    for task_suite in DIGITS_SUITES:
        print("\n" + "=" * 60)
        print(f"Phase 3 External Memory Boost Benchmark: suite={task_suite}")
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_external_memory_boost_{task_suite}_results.json",
...
def render_markdown(payload: Dict) -> str:
    lines = [
        "# Phase 3 External Memory Boost Benchmark",
        "",
        f"- Digits suites: `{', '.join(payload['task_suites'])}`",
        f"- Shared tuned configuration: current-path boost `{payload['current_path_lr_boost']:.2f}`",
...
def main() -> None:
    ...
    print(f"Saved external-memory boost benchmark JSON: {TARGET_JSON}")
    print(f"Saved external-memory boost benchmark markdown: {TARGET_MD}")
