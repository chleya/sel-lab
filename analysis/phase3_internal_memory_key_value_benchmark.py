# -*- coding: utf-8 -*-
"""
Compare key-value archived-memory retrieval against low-budget recency.
"""
...
REPRESENTATION_CASES = (
    ("full", 0, "recency"),
    ("recency_2", 2, "recency"),
    ("key_value_2", 2, "key_value"),
)
QUANT_BITS = 4
TARGET_JSON = resolve_canonical_results_path("phase3_internal_memory_key_value_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_INTERNAL_MEMORY_KEY_VALUE_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    ...
    for task_suite in DIGITS_SUITES:
        ...
        print("\n" + "=" * 60)
        print(
            "Phase 3 Internal Memory Key-Value Benchmark: "
            f"suite={task_suite}, bits={QUANT_BITS}, budget=2"
        )
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_internal_memory_key_value_2_{task_suite}_results.json",
            policies=("adapt_only", "task_specialist_clone_current_path_boost_merge"),
        )
        suite_row["key_value_2"] = result["policies"]["task_specialist_clone_current_path_boost_merge"]["summary"]
...
def main() -> None:
    ...
    print(f"Saved internal-memory key-value benchmark JSON: {TARGET_JSON}")
    print(f"Saved internal-memory key-value markdown: {TARGET_MD}")
