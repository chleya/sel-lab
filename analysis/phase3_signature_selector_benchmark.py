# -*- coding: utf-8 -*-
"""
Benchmark a multi-signal task-signature regime selector against hardcoded regime switching and prior selector baselines.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_signature_selector_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_SIGNATURE_SELECTOR_BENCHMARK.md"
...
def run_benchmark() -> Dict:
    payload = run_policy_benchmark_family(
        ...
        result_prefix="phase3_signature_selector",
        title="Phase 3 Signature Selector Benchmark",
        config_overrides={
...
def render_markdown(payload: Dict) -> str:
    lines = [
        "# Phase 3 Signature Selector Benchmark",
        "",
        "Multi-signal task-signature regime selector benchmarked against hardcoded regime switching and prior selector baselines.",
        "",
        f"- Family: `{payload['family_name']}`",
        f"- Overall best policy: `{payload['overall_best_policy']}`",
        (
            f"- Signature thresholds: conflict `{payload['selector_config']['selector_conflict_threshold']:.2f}`, "
...
def main() -> None:
    ...
    print(f"Saved signature selector benchmark JSON: {TARGET_JSON}")
    print(f"Saved signature selector benchmark markdown: {TARGET_MD}")
