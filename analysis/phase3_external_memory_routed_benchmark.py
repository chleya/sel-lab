# -*- coding: utf-8 -*-
"""
Evaluate routed retrieval on the external-memory boost mechanism.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_external_memory_routed_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_EXTERNAL_MEMORY_ROUTED_BENCHMARK.md"
...
def build_config(task_suite: str) -> AblationConfig:
    return replace(
        ...
        specialist_confidence_threshold=0.35,
        specialist_route_min_scale=0.05,
...
def run_benchmark() -> Dict:
    ...
    for task_suite in DIGITS_SUITES:
        print("\n" + "=" * 60)
        print(f"Phase 3 External Memory Routed Benchmark: suite={task_suite}")
        print("=" * 60)
        result = run_phase3_ablations(
            config=build_config(task_suite),
            result_filename=f"phase3_external_memory_routed_{task_suite}_results.json",
...
def render_markdown(payload: Dict) -> str:
    lines = [
        "# Phase 3 External Memory Routed Benchmark",
        "",
        f"- Digits suites: `{', '.join(payload['task_suites'])}`",
        f"- Shared tuned configuration: current-path boost `{payload['current_path_lr_boost']:.2f}`, route threshold `{payload['specialist_confidence_threshold']:.2f}`",
...
def main() -> None:
    ...
    print(f"Saved external-memory routed benchmark JSON: {TARGET_JSON}")
    print(f"Saved external-memory routed benchmark markdown: {TARGET_MD}")
