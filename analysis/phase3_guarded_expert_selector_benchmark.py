# -*- coding: utf-8 -*-
"""
Preserve the sparse boost branch and the embedded signature branch, and learn only the default-region quadratic selector.
"""
...
TARGET_JSON = resolve_canonical_results_path("phase3_guarded_expert_selector_benchmark.json")
TARGET_MD = PROJECT_ROOT / "PHASE3_GUARDED_EXPERT_SELECTOR_BENCHMARK.md"
...
def build_config(
    ...
    cfg.selector_guarded_expert_default_fit_weights = tuple(
        selector_payload.get("weights", (0.0,) * TASK_REGIME_QUADRATIC_FEATURE_COUNT)
    )
    cfg.selector_guarded_expert_default_fit_bias = float(selector_payload.get("bias", 0.0))
    return cfg
...
def fit_selector_payload() -> Dict[str, object]:
    ...
    payload["quadratic_feature_count"] = TASK_REGIME_QUADRATIC_FEATURE_COUNT
    return payload
...
def run_benchmark() -> Dict:
    ...
    for benchmark_name in benchmark_names:
        ...
        print("\n" + "=" * 60)
        print(f"Phase 3 Guarded Expert Selector Benchmark: {benchmark_name} ({task_suite})")
        print("=" * 60)
        result = run_phase3_ablations(
            ...
            result_filename=f"phase3_guarded_expert_selector_{benchmark_name}_{task_suite}.json",
            policies=policies,
        )
...
def render_markdown(payload: Dict) -> str:
    lines = [
        "# Phase 3 Guarded Expert Selector Benchmark",
        "",
        "Guarded expert selector that preserves the hard sparse branch and embedded signature branch while learning only the default-region quadratic head.",
        "",
...
def main() -> None:
    ...
    print(f"Saved guarded expert selector benchmark JSON: {TARGET_JSON}")
    print(f"Saved guarded expert selector benchmark markdown: {TARGET_MD}")
