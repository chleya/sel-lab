...
def run_phase2_improved():
    return run_phase2_suite(
        title="Phase 2c: Improved Evolution with Knowledge Reuse",
        config=Phase2Config(output_size=3, learning_rate=0.03, runs=5, epochs=100),
        task_fn=create_hard_task,
        evolving_builder=lambda config, seed: EvolvingNetwork(config, seed=seed, use_knowledge_reuse=True),
        result_filename="phase2_improved_results.json",
        success_threshold=0.02,
        extra_payload={"method": "knowledge_reuse"},
    )
